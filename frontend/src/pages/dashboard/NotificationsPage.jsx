...
const typeLabels = {
  ...
  article_approved: 'News Approved',
  article_rejected: 'News Rejected',
  article_submitted: 'News Submitted',
...
export function NotificationsPage() {
  ...
  useEffect(() => {
    ...
    if (!rejectId || notifications.length === 0) return;
    const notification = notifications.find((n) => n._id === rejectId);
    if (notification) {
      setRejectModal(notification);
      if (!notification.isRead) {
        markAsRead(notification._id);
      }
    }
  }, [notifications, searchParams, markAsRead]);
  ...
  return (
    <>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl sm:text-2xl font-bold text-dark-900 dark:text-white">Notifications</h1>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {resolvedLink && notification.type !== 'article_rejected' && (
                      <Link
                        to={resolvedLink}
      ...
      >
        <div className="space-y-3">
          ...
          {rejectModal?.metadata?.reason && (
            <div className="rounded-lg bg-red-50 dark:bg-red-950/30 p-3 text-sm text-red-700 dark:text-red-300">
              {rejectModal.metadata.reason}
            </div>
          )}
          <div className="flex justify-end gap-2 pt-2">
            <Button variant="secondary" onClick={() => setRejectModal(null)}>Close</Button>
            {rejectModal?.relatedArticle?._id && (
              <Link to={`/dashboard/articles/${rejectModal.relatedArticle._id}/edit`}>
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <p className="text-sm text-dark-500 mb-3">
              Email notifications are sent for important updates like news approvals and new comments.
            </p>
            <div className="space-y-2">
              {[
                { key: 'articlePublished', label: 'News Published' },
                { key: 'articleApproved', label: 'News Approved/Rejected' },
