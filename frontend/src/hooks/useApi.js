...
export function useArticles(params = {}) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (q && q.length >= 2) {
        const response = await articlesAPI.search({ q, ...otherParams });
        return response.data;
      }
      // Otherwise use the regular getAll endpoint
      const response = await articlesAPI.getAll(otherParams);
      return response.data;
    },
  });
}
...
export function useReplyContact() {
  ...
  return useMutation({
    mutationFn: ({ id, message }) => contactAPI.reply(id, { message }),
    onSuccess: () => {
...
export function usePublishedPages() {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await pagesAPI.getPublished();
...
export function useDuplicatePage() {
  ...
  return useMutation({
    mutationFn: (id) => pagesAPI.duplicate(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['pages'] });
