        ...
        <div className="flex-1 min-w-0 flex flex-col justify-between py-1">
          {category && (
            <Link to={`/category/${category.slug}`}>
              <Badge
                className="inline-flex items-center text-[10px] px-2 py-0.5"
                style={{
                  backgroundColor: `${category.color || getCategoryAccent(category.name)}20`,
                  color: category.color || getCategoryAccent(category.name),
                }}
              >
                {category.name}
              </Badge>
      ...
      <div className={cn('p-4 sm:p-5', isFeatured && 'lg:py-6')}>
        ...
        {category && (
          <Link to={`/category/${category.slug}`} className="inline-block mb-2 sm:mb-3">
            <Badge
              className="inline-flex items-center hover:opacity-80 transition-opacity text-xs"
              style={{
                backgroundColor: `${category.color || getCategoryAccent(category.name)}20`,
                color: category.color || getCategoryAccent(category.name),
              }}
