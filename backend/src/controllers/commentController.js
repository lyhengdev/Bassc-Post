...
export const getArticleComments = asyncHandler(async (req, res) => {
  ...
  // Build query - show approved comments OR user's own pending comments
  const query = {
    article: articleId,
    parent: null,
  };
  ...
  if (req.user) {
    // Authenticated user: show approved + their own pending
    query.$or = [
      { status: 'approved' },
  ...
  const comments = await Comment.find(query)
    ...
    .populate({
      path: 'replies',
      match: req.user 
        ? { $or: [{ status: 'approved' }, { author: req.user._id, status: 'pending' }] }
        : { status: 'approved' },
