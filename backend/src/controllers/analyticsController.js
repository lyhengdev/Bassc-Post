...
export const getAdsAnalytics = asyncHandler(async (req, res) => {
  ...
  const daily = await AdStatsDaily.aggregate([
    { $match: { date: { $gte: startDate } } },
    {
      $group: {
  ...
  const totalImpressions = daily.reduce((sum, row) => sum + (row.impressions || 0), 0);
  const totalClicks = daily.reduce((sum, row) => sum + (row.clicks || 0), 0);
